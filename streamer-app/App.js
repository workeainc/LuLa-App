...
// 🔥 CRITICAL: Import Firebase configuration FIRST to ensure proper initialization
...
import { app as firebaseApp, messaging as messagingService } from './firebaseConfig';
...
export const testIncomingCall = async (userId) => {
  try {
    ...
    const result = await AuthService.update(userId, { 
  ...
  useEffect(() => {
    ...
    if (user?.id) {
      setupFCM();
          ...
          // Periodic Firebase connection check - less aggressive
    const connectionCheckInterval = setInterval(() => {
      // Only check if Firebase is not available and we're not already retrying
